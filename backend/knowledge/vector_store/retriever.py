...
import time
import threading
from core.memory.memory_manager import get_context_for_embedding
    ...
    if _vectordb is None:
        with _lock:
            if _vectordb is None:
                ...
                print("STEP 6: Vector store checked")
...
def retrieve_reflection_unit(user_input: str) -> dict:
    """
    Retrieve optional reflection context for natural model reasoning.
    ...
    Returns:
        dict: {
            "reframe": str (optional),
            "cognitive_pattern": str (optional)
        }
    """
    try:
        print("---- SOFT RETRIEVAL START ----")
        print("User Input:", user_input)
        
        # Get context-aware embedding
        context = get_context_for_embedding()
        expanded_query = f"""
...
""".strip()
        ...
        try:
            ...
            print("Vector Results Count:", len(results))
            ...
            if results:
                ...
                print("Retrieved Reframe:", reframe[:100] if reframe else "None")
                print("Retrieved Pattern:", cognitive_pattern)
                ...
                print("No retrieval results - returning empty context")
                return {
                    "reframe": "",
                    "cognitive_pattern": ""
                }
