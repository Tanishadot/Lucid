...
class ReflectionEngine:
    ...
    def generate_reflection(self, message: str, session_context: List[Dict]) -> str:
        # Analyze the message to determine appropriate question type
        question_type = self._classify_message(message)
        
        # Get appropriate templates
        templates = self.question_templates[question_type]
        
        # Select a question (avoid repetition from recent messages)
        available_questions = self._avoid_repetition(templates, session_context)
        
        if not available_questions:
            available_questions = templates
